...
@router.get("/{group-id}", response_model=schemas.Group, status_code=200)
def get_group(group_id: int, db: Session = Depends(get_db)):
    group = crud.get_group(db=db, group_id=group_id)
    if group==None:
        error = {"code": "RESOURCE_NOT_FOUND", "message": "The group resource doesn't exists. Verify id {}".format(group_id) }
...
@router.put("/{group-id}",  status_code=200)
def update_group(group_id: int, group: schemas.GroupCreate, db: Session = Depends(get_db)):
    group = crud.update_group(db=db, group_id=group_id, group=group)
    return group
